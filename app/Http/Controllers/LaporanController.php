...
class LaporanController extends Controller
{
    ...
    public function index()
    {
        ...
        $laporan = Sewa::where('status_id', 8)->where('pemilik_id', Auth::id())->whereBetween('created_at', [$start, $end])->get();
    ...
    public function cetak($daterange)
    {
        ...
        $sewa = Sewa::where('status_id', 8)->where('pemilik_id', Auth::id())->whereBetween('created_at', [$start, $end])->get();

        //LOAD VIEW UNTUK PDFNYA DENGAN MENGIRIMKAN DATA DARI HASIL QUERY
        $pdf = PDF::loadView('vendor.laporan.cetakLaporan', compact('sewa', 'date'));
        //GENERATE PDF-NYA
        return $pdf->stream();
    }
}
